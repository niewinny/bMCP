"""
HTTP MCP Server using MCP SDK

This module provides an HTTP-based Model Context Protocol (MCP) server.
When running inside Blender, it uses bpy for main-thread execution.
For standalone use, a BaseServerManager is provided.
...
- All bpy operations run on Blender's main thread via bpy.app.timers (when available)
...
class ServerManager:
    ...
    Requires bpy (Blender Python) for main-thread execution bridging.
    Use this class when running inside Blender.
    """

    def __init__(self, get_config_fn=None):
        """
        Args:
            get_config_fn: Optional callable that returns server config tuple:
                (network_access, port, enable_logs, auth_token, auth_required).
                If None, uses bpy.context.preferences (requires bpy).
        """
        if not _HAS_BPY and get_config_fn is None:
            raise RuntimeError(
                "ServerManager requires bpy (Blender Python) or a get_config_fn. "
                "Use get_config_fn parameter for standalone applications."
            )
    ...
    def _get_server_config(self):
        """
        Get server configuration from preferences or config function.

        Returns:
            tuple: (network_access, port, enable_logs, auth_token, auth_required)
        """
        if self._get_config_fn is not None:
            return self._get_config_fn()

        # Default: read from bpy preferences
        # Find addon preferences by searching for the addon that uses bmcp
        addon_prefs = None
        if _HAS_BPY:
    ...
    def start(self):
        ...
        try:
            ...
            # Store masked token for logging
